...
Phases 0 (setup), 1 (ingestion) and 2 (retrieval) are done. `config/settings.py` and `config/logging_config.py` are implemented and tested, and `.github/workflows/ci.yml` runs lint, types, unit tests, integration tests against a Chroma service container, and a secret scan. The evaluation data and retrieval metrics are also in place. `data/raw/` holds a 14-document corpus on RAG and LLM evaluation; each document's filename is its doc id. `data/golden_dataset/qa_pairs.json` has 30 examples, and `evaluation/golden_dataset.py` and `evaluation/metrics.py` are implemented. A unit test fails if a golden example references a document that is missing from `data/raw/`, or if a corpus document has no golden question. So when you add, rename or remove a corpus document, update the golden dataset at the same time. Ingestion is implemented in `ingestion/`: `loaders`, `chunking`, `chunk_io`, `embedding` and `cli`, run through `scripts/run_ingestion.py`. Retrieval is implemented in `retrieval/` (`vector_store`, `reranker`, `retriever`, `seed`), and retrieval evaluation in `evaluation/evaluator.py` and `evaluation/cli.py`. Everything else is still a placeholder holding only a docstring or a `# TODO`: generation, the observability and API modules, `evaluation_gate.yml` and `docker/Dockerfile`. The intended design is written up in `docs/architecture.md` and `docs/evaluation_methodology.md`. Treat those two documents as the spec when implementing a module. The tools for each phase, and whether each is in use, chosen or only proposed, are listed in the "Tooling by phase" table in `docs/architecture.md`. Add a dependency only when its phase starts, and ask before adding a *proposed* one.
...
uv sync --all-extras --group notebook                    # + JupyterLab and pandas (plain `uv sync` removes them)
docker compose -f docker/docker-compose.yml up -d        # start Chroma (localhost:8001); `down` to stop
...
docker compose -f docker/docker-compose.yml --profile ui up -d   # + chromadb-admin UI on localhost:3001 (connect to http://chroma:8000)
uv run --group notebook jupyter lab                      # notebooks/exploration.ipynb: browse chunks, ask questions, rerank experiment
...
- **Chroma runs in Docker, never embedded.** It is a server from `docker/docker-compose.yml`, pinned to the same version as the `chromadb-client` package. Keep the image tag and the client version in step. `ChromaVectorStore` talks to it over HTTP. `replace_all` drops and recreates the collection with cosine distance, so re-seeding never leaves stale chunks. Search scores are cosine similarities (`1 - distance`), and qdrant raises `NotImplementedError`.
- **The pipeline is three commands:** ingest, then seed, then evaluate. `chunks.jsonl` is the hand-off file between ingestion and seeding. Re-run seeding after any change to chunking or the embedding model. The evaluator maps retrieved chunks to their `doc_id` before scoring, and writes `reports/retrieval_report.json`, which is git-ignored.
- **Inspection tools.** `chromadb-admin` is a pinned community image under the `ui` compose profile. It is browse-only in practice: its search box looks up chunk ids, not questions. `notebooks/exploration.ipynb` covers searching by meaning and evaluation experiments through the project's own retriever. Commit the notebook with outputs cleared, and keep it passing `ruff check` and `ruff format`.
- **Optional embedding backends.** `sentence-transformers` (with CPU-only torch on Linux; also used for the cross-encoder re-ranker) and `openai` are extras, not core dependencies, so CI never installs torch. `embedding.py` imports them lazily, and tests use fakes that satisfy the `Embedder` protocol. Tests that load a real model are marked `integration` and skip when the extra is missing. mypy skips these packages so local and CI results match.
